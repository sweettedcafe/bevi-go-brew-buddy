...
begin
  ...
  with base as (
    select o.id, o.created_at, oi.qty,
           ...
           coalesce(o.txn_kind::text,'sale') as kind, o.status::text as status,
     ...
     where o.created_at >= v_from
       ...
       and coalesce(o.status::text,'') not in ('on_hold','open')   -- unpaid: excluded
    ...
    jsonb_build_object(
      'orders', coalesce((select count(distinct id) from signed
                           where kind not in ('void','refund')
                             and coalesce(status::text,'') not in ('voided','refunded')), 0),
