-- Phase 49: a bundle component can offer a choice between MULTIPLE ITEMS
-- (e.g. "any refresher OR any cookie"), each optionally with its own variants.
-- Run in the Supabase SQL editor. Additive / idempotent.

-- 1) allowed menu items per bundle component --------------------------------
alter table public.bundle_items
  add column if not exists menu_item_ids uuid[];

update public.bundle_items
   set menu_item_ids = array[menu_item_id]
 where menu_item_ids is null and menu_item_id is not null;

-- 2) public_menu exposes menu_item_ids --------------------------------------
...
returns jsonb language sql security definer set search_path=public as $$
  select jsonb_build_object(
    ...
    'bundle_items', coalesce((select jsonb_agg(jsonb_build_object(
        ...
        'menu_item_ids', coalesce(bi.menu_item_ids, array[bi.menu_item_id]),
...
-- 3) customer_self_order honours item + variant choices ---------------------
...
declare
  ...
  v_citem uuid; v_allowed_items uuid[]; v_ci public.menu_items%rowtype;
...
begin
  ...
  for v_bundle in select * from jsonb_array_elements(coalesce(p_payload->'bundles','[]'::jsonb)) loop
    ...
    for v_bi in
      select bi.id, bi.menu_item_id, bi.menu_item_ids, bi.qty, bi.variant_id, bi.variant_ids,
    ...
    loop
      ...
      v_allowed_items := coalesce(v_bi.menu_item_ids, array[v_bi.menu_item_id]);

      select nullif(c->>'variant_id','')::uuid, nullif(c->>'menu_item_id','')::uuid
        into v_choice, v_citem
        from jsonb_array_elements(coalesce(v_bundle->'choices','[]'::jsonb)) c
       where (c->>'bundle_item_id')::uuid = v_bi.id
       limit 1;

      -- defaults from the stored component
      ...
      if v_citem is not null then
        if not (v_citem = any (v_allowed_items)) then
          raise exception 'item not allowed for this bundle';
        end if;
        select * into v_ci from public.menu_items where id = v_citem and is_active;
        if not found then raise exception 'menu item unavailable'; end if;
        v_bbase := v_ci.price;
        v_bname := v_ci.name;
        v_variant_id := null;
      ...
      if v_choice is not null then
        ...
        v_bbase := v_cv.price;
        v_bname := coalesce(v_ci.name, v_bi.name) || ' — ' || v_cv.name;
      end if;
      ...
      values (v_order_id, coalesce(v_citem, v_bi.menu_item_id), v_variant_id, v_bname,
