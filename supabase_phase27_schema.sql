-- =====================================================================
-- BEVI & GO — Phase 27
--   1) Preserve order_no when a barista resumes a customer/held order,
--      so the number the customer sees on their phone matches the POS.
--   2) Add pos_notify_customer + customer_alerted_at so the ready alert
--      pops (with looping beep) on the customer's device page.
-- Idempotent; run after phase 26.
-- =====================================================================

alter table public.orders
  add column if not exists customer_alerted_at timestamptz;

-- Resume: keep the row (and its order_no), wipe items so the barista's
-- cart is rebuilt from scratch. pos_create_order handles the rest.
...
begin
  ...
  -- Wipe items; the row (and its order_no) stays. pos_create_order will
  -- rebuild order_items from the barista's final cart and mark completed.
...
-- pos_create_order accepts existing_order_id AND rebuilds items from the
-- payload so barista edits are respected while the order_no is preserved.
...
begin
  ...
  if v_existing_order_id is not null then
    -- Preserve the pre-assigned order_no; rebuild items from payload.
  ...
  -- Insert items (both new and resumed paths).
...
-- Notify: barista taps "Notify ready" → customer's device page starts alerting.
create or replace function public.pos_notify_customer(p_order_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_actor uuid := auth.uid();
begin
  if v_actor is null then raise exception 'not authenticated'; end if;
  if not public.is_staff(v_actor) then raise exception 'not authorized'; end if;
  update public.orders set customer_alerted_at = now() where id = p_order_id;
  if not found then raise exception 'order not found'; end if;
  return jsonb_build_object('ok', true);
end $$;
grant execute on function public.pos_notify_customer(uuid) to authenticated;

-- Customer status: expose alerted flag so their phone page can start beeping.
...
begin
  ...
  return jsonb_build_object(
    ...
    'status',   v_o.status,
    'total',    v_o.total,
    'alerted',  (v_o.customer_alerted_at is not null)
  );
end $$;
